...
class PropertiesManager(models.Manager):
    ...
    def featured(self): #Property.objects.featured() 
        return self.get_queryset().featured()

    def get_by_id(self, id):
        qs = self.get_queryset().filter(id=id) # Property.objects == self.get_queryset()
        if qs.count() == 1:
            return qs.first()
        return None
...
class Properties(models.Model):
    ...
    def get_absolute_url(self):
        #return "/Property/{slug}/".format(slug=self.slug)
